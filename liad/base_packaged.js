...
$(document).ready(function(){setUpConfirmationForStockBookSelection();});function setUpConfirmationForStockBookSelection(){$('.stock_book_form').submit(function(){if($('input.use_stock_book').attr('checked')){var message='Are you sure you do not want to personalize THE OBAMA TIME CAPSULE?\n\nThis decision is final.  We will ship your book immediately if you click OK.';return confirm(message);}else{return true;}});}
var dirty_text=false;$(document).ready(function(){requestPendingImages();watchTextAreasAndFieldsForChanges();$('.box .button').live('click',function(){$('.toggleable').toggle();return false;});});var warnOnDirty=function(){if(dirty_text){return confirm("You have non-previewed changes in your customization.  Are you sure you want to continue and lose these changes?");}
return true;};function watchTextAreasAndFieldsForChanges(){$('.watch_for_dirty').live('change',function(){dirty_text=true;});}
function requestPendingImages(){$('.customization .pending').each(function(){var customization=$(this).parents('.customization');var customization_id=customization.attr('customization_id');reloadImage(customization_id)});setTimeout(requestPendingImages,2000);}
function reloadImage(customization_id){var url="/book/customizations/"+customization_id+"/image.js";$.ajax({type:"GET",url:url,dataType:"script"});}
function showPreviewForImage(image,coords){var $button=$('.cropped_button');$('#template_image').css("marginLeft",0);if(coords.w<1||isNaN(coords.w))coords.w=10;if(coords.h<1||isNaN(coords.h))coords.h=10;var rx=$button.attr('targetWidth')/coords.w;var ry=$button.attr('targetHeight')/coords.h;$button.css({width:Math.round(rx*image.width)+'px',height:Math.round(ry*image.height)+'px',marginLeft:'-'+Math.round(rx*coords.x)+'px',marginTop:'-'+Math.round(ry*coords.y)+'px',top:$button.attr('displayTop')+'px',left:$button.attr('displayLeft')+'px'});}
var curr_page=0;var page_prefix="http://www.theobamatimecapsule.com/images/look_inside/";var pages=[];var page_width=640;function add_page(name)
{pages.push(name);}
function start_turn()
{if(curr_page<pages.length-1)
{document.getElementById('bk_pgr').src=page_prefix+pages[curr_page+1];turn_page(page_width,1);}}
function turn_page(wdth,lft)
{if(wdth>0)
{var nxt_wd=wdth-32;var nxt_lf=lft+16;document.getElementById('bk_pgmv').style.width=nxt_wd+'px';document.getElementById('bk_pgmv').style.left=nxt_lf+'px';var t=setTimeout("turn_page("+nxt_wd+","+nxt_lf+")",25);}
else
{document.getElementById('bk_pgmv').src=page_prefix+pages[curr_page+1];document.getElementById('bk_pgl').style.zIndex=1;document.getElementById('bk_pgr').style.zIndex=3;finish_turn(0,page_width/2+1);}}
function finish_turn(wdth,lft)
{if(wdth<page_width)
{var nxt_wd=wdth+32;var nxt_lf=lft-16;document.getElementById('bk_pgmv').style.width=nxt_wd+'px';document.getElementById('bk_pgmv').style.left=nxt_lf+'px';var t=setTimeout("finish_turn("+nxt_wd+","+nxt_lf+")",25);}
else
{curr_page=curr_page+1;document.getElementById('bk_pgl').style.zIndex=3;document.getElementById('bk_pgl').src=page_prefix+pages[curr_page];document.getElementById('bk_pgmv').src=page_prefix+pages[curr_page];document.getElementById('bk_pgr').style.zIndex=1;}}
function start_back()
{if(curr_page>0)
{document.getElementById('bk_pgl').style.visibility='hidden';document.getElementById('bk_pgl').style.zIndex=1;document.getElementById('bk_pgr').src=page_prefix+pages[curr_page];document.getElementById('bk_pgl').src=page_prefix+pages[curr_page-1];document.getElementById('bk_pgr').style.zIndex=3;document.getElementById('bk_pgl').style.visibility='visible';back_page(page_width,1);}}
function back_page(wdth,lft)
{if(wdth>0)
{var nxt_wd=wdth-32;var nxt_lf=lft+16;document.getElementById('bk_pgmv').style.width=nxt_wd+'px';document.getElementById('bk_pgmv').style.left=nxt_lf+'px';var t=setTimeout("back_page("+nxt_wd+","+nxt_lf+")",25);}
else
{document.getElementById('bk_pgmv').src=page_prefix+pages[curr_page-1];document.getElementById('bk_pgl').style.zIndex=3;document.getElementById('bk_pgr').style.zIndex=1;finish_back(0,page_width/2+1);}}
function finish_back(wdth,lft)
{if(wdth<page_width)
{var nxt_wd=wdth+32;var nxt_lf=lft-16;document.getElementById('bk_pgmv').style.width=nxt_wd+'px';document.getElementById('bk_pgmv').style.left=nxt_lf+'px';var t=setTimeout("finish_back("+nxt_wd+","+nxt_lf+")",25);}
else
{curr_page=curr_page-1;document.getElementById('bk_pgl').style.zIndex=3;document.getElementById('bk_pgl').src=page_prefix+pages[curr_page];document.getElementById('bk_pgmv').src=page_prefix+pages[curr_page];document.getElementById('bk_pgr').style.zIndex=1;}}
$(document).ready(function(){$('.track_to_amazon').click(function(){try{pageTracker._trackPageview("/link_to_amazon");}
catch(x){}});});